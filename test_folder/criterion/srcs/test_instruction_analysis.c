...
Test(test_instruction_analysis, instruction_analysis_err1_test, .init=redirect_all_stdout3)
{
	...
	as->fd = open("test_folder/asm_test_folder/invalid_s_files/invalid_instruction_error_01.s", O_RDONLY);
	analyze_lexicon(as);	
	analyze_syntax(as);
...
Test(test_instruction_analysis, instruction_analysis_break1_test, .init=redirect_all_stdout3)
{
	...
	as->fd = open("test_folder/asm_test_folder/invalid_s_files/instruction_break01.s", O_RDONLY);
	analyze_lexicon(as);	
	analyze_syntax(as);
	ret = analyze_instructions(as);
