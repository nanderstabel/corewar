...
Test(test_parameter_analysis, parameter_analysis_err4_test, .init=redirect_all_stdout4)
{
	...
	as->fd = open("test_folder/asm_test_folder/invalid_s_files/parameter_error_04.s", O_RDONLY);
	analyze_lexicon(as);	
	analyze_syntax(as);
	analyze_instructions(as);
	ret = analyze_parameters(as);
...
Test(test_parameter_analysis, parameter_analysis_err5_test, .init=redirect_all_stdout4)
{
	...
	as->fd = open("test_folder/asm_test_folder/invalid_s_files/parameter_error_05.s", O_RDONLY);
	analyze_lexicon(as);	
	analyze_syntax(as);
	analyze_instructions(as);
	ret = analyze_parameters(as);
